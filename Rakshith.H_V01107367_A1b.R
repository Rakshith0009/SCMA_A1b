...
setwd('/Users/preethireddy/Downloads/untitled folder 2')
getwd()
...
data <- read.csv("/Users/preethireddy/Downloads/IPL_ball_by_ball_updated+till+2024.csv")
# Load required libraries
library(dplyr)
library(readr)
library(stringr)
...
# Load datasets
ipl_bbb <- read_csv('/Users/preethireddy/Downloads/IPL_ball_by_ball_updated+till+2024.csv')
ipl_salary <- read_excel('/Users/preethireddy/Downloads/IPL+SALARIES+2024.xlsx')
...
# Filter the runs scored by RD Chahar
RD_Chahar_runs <- runs %>% filter(Striker == "RD Chahar") %>% pull(runs_scored)
...
# Fit the distribution to RD Chahar's runs scored and get the best distribution
best_distribution <- get_best_distribution(RD_Chahar_runs)
